/*
 * uzcTest_Tdm_bt — Master (PlatformIO)
 * uzcTest_i2s_bt を ESP32-S3 + TDM 8ch TX に移植
 *
 * ESP32-S3 + SD_MMC + TDM 8ch
 *   WAV: 44.1kHz PCM16 再生 (play)
 *   UZC: mplay=Main MP3デコード→PCM, splay=圧縮TDM (Slaveデコード)
 *   TDM: 常時 44.1kHz。CH1=データ, CH8=制御タグ (AAAA/AA00/AA55/5500/FFFF/0000)
...
// TDM TX — uzuCastV3_Main と同一
// BCLK=5, WS=16, DOUT=4

// SD_MMC 4bit — uzuCastV3_Main と同一
...
static const int PIN_SD_CD  = 21;  // LOW = カード挿入
static const int PIN_LED    = 2;

static const uint32_t SERIAL_BAUD = 115200;
static const uint32_t SD_MMC_HZ = 20000000;
static const uint32_t TDM_SAMPLE_RATE = 44100;
...
// 44.1kHz で 1 UZC スロット期間分の TDM フレーム上限
static const size_t UZC_TDM_PERIOD_MAX = 1280;

// TDM CH8 制御タグ (Slave I2S R-ch と同一値)
#define I2S_TAG_RAW        TDM_TAG_RAW
#define I2S_TAG_MP3_START  TDM_TAG_MP3_START
#define I2S_TAG_MP3_END    TDM_TAG_MP3_END
#define I2S_TAG_PAD        TDM_TAG_PAD
#define I2S_TAG_SLOT_DATA  TDM_TAG_SLOT_DATA
#define I2S_TAG_INVALID    TDM_TAG_INVALID
// 1 MP3 フレーム期間 = 1152 PCM samples @ 44.1kHz
static const size_t UZC_MP3_PERIOD_FRAMES_441 = 1152;
...
static const uint32_t UZU_PCM_PLAY_CHANNEL = 0;  // CH1 (0-based) for TDM mono RAW path
...
static int16_t g_stest_tdm_pad[UZC_TDM_PERIOD_MAX * TDM_NUM_CH];
...
static void i2sWriteTaggedFrames(const int16_t* pairs, size_t stereoFrameCount) {
  tdmWriteTaggedStereoPairs(pairs, stereoFrameCount);
}

static void i2sWriteInvalidKeepalive(size_t frameCount) {
  tdmWriteInvalidKeepalive(frameCount);
}

static void i2sWriteRawTaggedMono(const int16_t* mono, size_t sampleCount) {
  tdmWriteRawMono(mono, sampleCount);
}

static bool initTdm() {
  return tdmTxInit(TDM_SAMPLE_RATE) && tdmTxEnsureEnabled();
}

static bool setTdmTxRate(uint32_t rate) {
  return tdmTxSetRate(rate);
}
...
static bool initSd() {
  ...
  bool ok = SD_MMC.begin("/sdcard", false, false, SD_MMC_HZ);
...
static File openSdFile(const char* path) {
  ...
  if (path[0] != '/') {
    ...
    return SD_MMC.open(withSlash, FILE_READ);
  }
  return File();
}
...
static bool validateUzuPcmForPlay(File& f, UzuPcmInfo& info) {
  ...
  if (info.sampleRate != TDM_SAMPLE_RATE) {
...
static bool validateUzcForPlay(File& f, UzcHeader& hdr) {
  ...
  if (hdr.sampleRate != TDM_SAMPLE_RATE) {
    Serial.print(F("error: sample rate "));
    Serial.print(hdr.sampleRate);
    Serial.println(F(" Hz (require 44100 Hz)"));
    return false;
...
static bool playUzuPcmStream(File& f, const UzuPcmInfo& info) {
  if (!setTdmTxRate(TDM_SAMPLE_RATE)) {
    return false;
  }

  Serial.print(F("UZU PCM TDM RAW (CH8=AAAA) CH"));
...
static void i2sWritePcm(const uint8_t* data, size_t nbytes, uint16_t channels) {
  ...
  for (size_t i = 0; i < frames; i++) {
    monoBuf[off++] = (channels == 2) ? in[i * 2] : in[i];
    if (off >= 256) {
      if (!tdmWriteRawMono(monoBuf, off)) {
        Serial.println(F("error: TDM PCM write failed"));
        return;
      }
      off = 0;
    }
...
static void pcmPaceAfterWrite(size_t stereoFrames) {
  ...
  const uint64_t dueSamples = (elapsedMs * (uint64_t)TDM_SAMPLE_RATE) / 1000ULL;
  if (g_pcmPaceSamplesSent > dueSamples) {
    const uint32_t waitMs =
        (uint32_t)(((g_pcmPaceSamplesSent - dueSamples) * 1000ULL) / (uint64_t)TDM_SAMPLE_RATE);
...
static bool playUzcStream(File& f, const UzcHeader& hdr) {
  ...
  const size_t periodFrames = UZC_MP3_PERIOD_FRAMES_441;
  const size_t taggedPayloadFrames = slotStereoFrames + 2U;
  const size_t invalidPadFrames =
      (periodFrames > taggedPayloadFrames) ? (periodFrames - taggedPayloadFrames) : 0;

  if (periodFrames > UZC_TDM_PERIOD_MAX) {
    Serial.println(F("error: UZC TDM period buffer too small"));
    return false;
  }
  bool ok = false;
  ...
  pcmPaceReset();
  Serial.print(F("UZC TDM tagged MP3 @ "));
  Serial.print(TDM_SAMPLE_RATE);
  Serial.print(F(" Hz period "));
  Serial.print(periodFrames);
  ...
  g_uzc_tx_frame_ms = (uint16_t)((1152ULL * 1000ULL) / TDM_SAMPLE_RATE);

  Serial.print(F("UZC TDM tag MP3 START txSlot="));
  Serial.print(txSlotSize);
  Serial.print(F(" maxFrame="));
  Serial.print(hdr.maxFrameSize);
  Serial.print(F(" periodF="));
  Serial.println(periodFrames);
  ...
  for (uint32_t frameNo = g_uzcFrameIndex; frameNo < hdr.totalFrameCount; frameNo++) {
    ...
    fillMp3TdmPeriodTaggedMono(g_stest_ch1_words, n1, g_stest_tdm_pad, periodFrames, slotStereoFrames);
    if (frameNo == 0) {
      Serial.print(F("UZC TX slot bytes[0..7]="));
      for (int i = 0; i < 8; i++) {
        Serial.printf(" %02X", g_uzc_ch1_tx_buf[i]);
      }
      Serial.println();
    }

    const size_t slotPartFrames = slotStereoFrames + 2U;
    const size_t padPartFrames = periodFrames - slotPartFrames;
    (void)slotPartFrames;
    (void)padPartFrames;
    if (!tdmWriteFrames(g_stest_tdm_pad, periodFrames)) {
      Serial.println(F("error: UZC TDM period write failed"));
      goto uzc_tx_done;
    }
    pcmPaceAfterWrite(periodFrames);
...
static bool startPlaybackRoute(const char* path, const char* label, PlayUzcRoute route) {
  ...
  const uint32_t taskStack =
      (route == PLAY_ROUTE_UZC_MASTER || route == PLAY_ROUTE_UZC_SLAVE) ? 32768U : 16384U;
...
static void cmdStest(const char* arg) {
  ...
  if (!initTdm()) {
    Serial.println(F("error: TDM init failed"));
    f.close();
    fileAccessEnd();
    return;
  }
...
static void cmdHelp() {
  Serial.print(F("uzcTest_Tdm_bt build "));
  ...
  Serial.println(F("mplay - UZCをMainでMP3デコード→PCM (TDM CH8=AAAA)"));
  Serial.println(F("splay - UZCを圧縮TDM転送 (CH8タグ / Slaveデコード)"));
...
void setup() {
  ...
  delay(500);
  Serial.println();
  Serial.print(F("uzcTest_Tdm_bt Master CLI ready  build "));
  Serial.println(BUILD_NUMBER);
  ...
  if (initTdm()) {
    Serial.println(F("TDM OK (play=WAV, mplay=PCM, splay=UZC TDM)"));
  } else {
