...
set "BAT_DIR=%~dp0"
set "PROJECT_DIR=%BAT_DIR%master"
...
:args_done
if "%UPLOAD_PORT%"=="" set "UPLOAD_PORT=COM11"
...
echo.
echo [sintest_i2s_mp3 Master] ESP32 on %UPLOAD_PORT%
echo [sintest_i2s_mp3 Master] Stage 2: I2S TX RAW/MP3 -^> Slave -^> BT
echo [sintest_i2s_mp3 Master] Project: master, env=%PIO_ENV%
echo [sintest_i2s_mp3 Master] Upload baud: %UPLOAD_SPEED% bps
echo.
...
:speed_without_s
echo [ERROR] Baud rate requires /s: writemaster.bat COM7 /s %UPLOAD_PORT%
goto show_help_error
...
:show_help
echo.
echo sintest_i2s_mp3 - writemaster.bat
echo   Build and upload Stage-2 Master firmware (I2S RAW/MP3 modes).
echo.
echo Usage:
echo   writemaster.bat [COMx] [/s baud]
echo.
echo Options:
echo   COMx          Serial port (default: COM7)
