...
set "BAT_DIR=%~dp0"
set "PROJECT_DIR=%BAT_DIR%uzuCastV3_Main"
...
echo.
echo [Main] UzuCast V3 Main - ESP32-S3 board required on %UPLOAD_PORT%
echo [Main] Project: uzuCastV3_Main, env=%PIO_ENV%
echo [Main] Platform: pioarduino (shared with Sub)
echo [Main] Framework: Arduino ESP32 3.3.9
echo [Main] Upload baud: %UPLOAD_SPEED% bps
echo.
...
:speed_without_s
echo [ERROR] Baud rate requires /s: writemaster.bat COM4 /s %UPLOAD_PORT%
goto show_help_error
...
:show_help
echo.
echo UzuCast V3 Main - writemaster.bat
echo   Build and upload firmware to the ESP32-S3 Main board.
echo.
echo Usage:
echo   writemaster.bat [COMx] [/s baud]
echo.
echo Options:
echo   COMx          Serial port (default: COM5)
...
echo.
echo Examples:
echo   writemaster.bat
echo   writemaster.bat COM7
echo   writemaster.bat /s 115200
echo   writemaster.bat COM7 /s 460800
echo.
exit /b 0
