/*
 * sintest_i2s_spk — Master (ESP32)
 * Stage 1: Master I2S -> Slave I2S -> Speaker
 *
...
static void printTestCatalog() {
  ...
  Serial.println(F("  1   Stage1: 440Hz sine on L, R=0xAAAA (Slave RAW tag path)"));
...
static void printMenu() {
  Serial.println();
  Serial.println(F("=== sintest_i2s_spk MASTER CLI ==="));
  Serial.print(F("Build: "));
  Serial.println(BUILD_NUMBER);
  Serial.println(F("Path: Master I2S -> Slave I2S -> Speaker (Stage 1)"));
...
void setup() {
  ...
  Serial.println();
  Serial.print(F("sintest_i2s_spk Master CLI ready  build "));
  Serial.println(BUILD_NUMBER);
  printMenu();
  printPrompt();
